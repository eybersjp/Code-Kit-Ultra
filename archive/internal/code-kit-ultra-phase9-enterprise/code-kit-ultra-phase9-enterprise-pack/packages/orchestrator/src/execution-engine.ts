...
import { evaluatePolicy, loadPolicyProfile } from "../../policy/src/index";
...
async function executeTask(bundle: RunBundle, task: PlanTask, index: number): Promise<{ completed: boolean; paused?: boolean }> {
  ...
  const policyResult = evaluatePolicy(task, policy);
  ...
  if (!policyResult.allowed) {
    ...
    upsertAdapterSummary(bundle, {
      ...
      status: "failed",
      attempts: 1,
      output: policyResult.reason ?? "Blocked by policy",
    });
    appendAuditEvent({ runId: bundle.state.runId, actor: "system", role: "system", action: "POLICY_BLOCK", stepId: task.id, details: { reason: policyResult.reason } });
    markState(bundle, "failed", { currentStepIndex: index, pauseReason: policyResult.reason });
    return { completed: false };
  }

  if ((task.requiresApproval || policyResult.requiresApproval) && !bundle.state.approved) {
    markState(bundle, "paused", {
      currentStepIndex: index,
      approvalRequired: true,
      pauseReason: policyResult.reason ?? `Approval required before step ${task.id}`,
    });
    pushStepLog(bundle, {
      stepId: task.id,
      title: task.title,
    ...
    appendAuditEvent({ runId: bundle.state.runId, actor: "system", role: "system", action: "APPROVAL_REQUIRED", stepId: task.id, details: { reason: bundle.state.pauseReason } });
  ...
  if (!adapter) {
    ...
    appendAuditEvent({ runId: bundle.state.runId, actor: "system", role: "system", action: "ADAPTER_NOT_FOUND", stepId: task.id, details: { adapterId: task.adapterId } });
  ...
  if (!validated) {
    ...
    appendAuditEvent({ runId: bundle.state.runId, actor: "system", role: "system", action: "VALIDATION_FAILED", stepId: task.id, details: { adapterId: task.adapterId } });
    markState(bundle, "failed", { currentStepIndex: index });
    return { completed: false };
  }

  const maxAttempts = Math.max(task.retryPolicy?.maxAttempts ?? 1, policy.rules.maxRetries || 1);
  let success = false;

  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    const startedAt = nowIso();
    try {
      appendAuditEvent({ runId: bundle.state.runId, actor: "system", role: "system", action: "STEP_EXECUTION_STARTED", stepId: task.id, details: { attempt, adapterId: task.adapterId } });
      ...
      appendAuditEvent({ runId: bundle.state.runId, actor: "system", role: "system", action: "STEP_EXECUTION_SUCCEEDED", stepId: task.id, details: { attempt, output } });
      ...
      appendAuditEvent({ runId: bundle.state.runId, actor: "system", role: "system", action: "STEP_EXECUTION_FAILED", stepId: task.id, details: { attempt, error: message } });

      if (attempt === maxAttempts) {
        if (task.rollbackPayload && adapter.rollback && policy.rules.allowRollback) {
...
export async function executeRunBundle(bundle: RunBundle): Promise<RunBundle> {
  ...
  for (let index = bundle.state.currentStepIndex; index < bundle.plan.tasks.length; index += 1) {
    ...
    if (!result.completed) return bundle;
  }
...
export async function retryTask(runId: string, targetStepId?: string): Promise<RunBundle> {
  ...
  bundle.state.updatedAt = nowIso();
  updateRunState(bundle.state.runId, bundle.state);
  appendAuditEvent({ runId, actor: "system", role: "system", action: "STEP_RETRY_REQUESTED", stepId: stepId, details: { currentStepIndex: index } });
...
export async function rollbackTask(runId: string, targetStepId?: string): Promise<RunBundle> {
  ...
  appendAuditEvent({ runId, actor: "system", role: "system", action: "ROLLBACK_COMPLETED", stepId: task.id, details: { automatic: false } });
