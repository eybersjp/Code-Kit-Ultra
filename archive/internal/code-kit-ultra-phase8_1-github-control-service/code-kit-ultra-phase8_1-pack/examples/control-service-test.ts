...
async function main(): Promise<void> {
  ...
  const runs = await fetch(`${base}/runs`).then((r) => r.json() as Promise<Array<{ runId: string }>>);
  if (!runs.find((run) => run.runId === bundle.state.runId)) {
    throw new Error("Run not returned by /runs endpoint.");
  }

  const approvals = await fetch(`${base}/approvals`).then((r) => r.json() as Promise<Array<{ runId: string }>>);
  if (!approvals.find((run) => run.runId === bundle.state.runId)) {
    throw new Error("Approval not returned by /approvals endpoint.");
  }

  const approvedRun = await fetch(`${base}/runs/${bundle.state.runId}/approve`, { method: "POST" }).then((r) =>
    r.json() as Promise<{ state: { status: string } }>,
  );
