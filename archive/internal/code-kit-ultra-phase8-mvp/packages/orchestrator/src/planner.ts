...
export function buildInitialPlan(input: UserInput): PlanTask[] {
  ...
  return [
    ...
    },
    {
      ...
      description: "Stage the next implementation wave for GitHub/API/AI adapters.",
      doneDefinition: "Backlog artifact exists.",
      taskType: "provider-backlog",
      adapterId: "github",
      payload: {
        nextSteps: ["Wire GitHub adapter", "Wire API adapter", "Wire AI adapter", "Add rollback and retry observability"],
      },
      retryPolicy: { maxAttempts: 2 },
    },
  ];
