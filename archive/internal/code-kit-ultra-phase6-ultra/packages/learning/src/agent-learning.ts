...
import type { AgentProfile, RunOutcomeInput, SpecialistAgent } from "../../shared/src/phase6-types";
...
export function updateAgentProfiles(
  ...
  for (const snapshot of outcome.agentDecisions) {
    ...
    const updated = {
      ...
      successes: existing.successes + (outcome.result === "success" ? 1 : 0),
      failures: existing.failures + (outcome.result === "failure" ? 1 : 0),
      lastUpdatedAt: now,
    };
