...
async function runCase(testCase: SmokeCase): Promise<void> {
  ...
  const artifactDirectory = (result as Record<string, unknown>).artifactDirectory as string;
  const artifactReportPath = (result as Record<string, unknown>).artifactReportPath as string;
  const memoryPath = (result as Record<string, unknown>).memoryPath as string;
  const report = (result as Record<string, unknown>).report as PersistedReport;
