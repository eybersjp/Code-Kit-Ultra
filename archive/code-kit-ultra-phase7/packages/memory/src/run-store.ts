...
interface ProjectMemory {
  ...
  executionHistory: Array<{ adapter: string; taskType: string; mode: string; ok: boolean; latencyMs?: number; at: string }>;
}

function memoryPath() { return path.resolve(".codekit/memory/project-memory.json"); }

export function loadProjectMemory(): ProjectMemory {
  const file = memoryPath();
  if (!fs.existsSync(file)) {
    const seed: ProjectMemory = { acceptedAssumptions: [], successfulSkillMappings: {}, successfulAdapterSelections: {}, adapterSuccessCounts: {}, executionHistory: [] };
...
export function updateMemoryFromRun(report: RunReport) {
  ...
  for (const a of report.assumptions) if (!mem.acceptedAssumptions.includes(a.text)) mem.acceptedAssumptions.push(a.text);
  for (const s of report.selectedSkills) mem.successfulSkillMappings[s.skillId] = (mem.successfulSkillMappings[s.skillId] || 0) + 1;
...
export function recordExecution(adapter: string, taskType: string, mode: string, ok: boolean, latencyMs?: number) {
  const mem = loadProjectMemory();
  mem.executionHistory.push({ adapter, taskType, mode, ok, latencyMs, at: new Date().toISOString() });
  if (ok) mem.adapterSuccessCounts[adapter] = (mem.adapterSuccessCounts[adapter] || 0) + 1;
  fs.writeFileSync(memoryPath(), JSON.stringify(mem, null, 2), "utf-8");
}
...
export function saveRunReport(report: RunReport, artifactDir?: string): string {
  ...
  const fullPath = path.join(dir, "run-report.json");
