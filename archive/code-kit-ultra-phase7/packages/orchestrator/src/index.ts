...
export async function runVerticalSlice(input: UserInput): Promise<RunReport> {
  ...
  const routes = [];
  for (const taskType of taskTypes) routes.push(await selectAdapter(taskType));

  const execution = [
    await executeTask("planning", { idea: input.idea }, Boolean(input.dryRun)),
    await executeTask("implementation", { idea: input.idea }, Boolean(input.dryRun))
  ];
  ...
  const gates = [
    ...
    evaluateGate("architecture", input.mode, "pass", "Architecture gate passes."),
    evaluateGate("deployment", input.mode, input.dryRun ? "needs-review" : "pass", input.dryRun ? "Dry-run stops before deployment." : "Deployment path not blocked.")
  ];

  const summary = `Mode: ${input.mode}. Plan tasks: ${plan.length}. Skills: ${selectedSkills.length}. Routes: ${routes.length}. Executions: ${execution.length}. Gates: ${gates.length}.`;
  const report: RunReport = { input, assumptions, clarifyingQuestions, plan, selectedSkills, routes, gates, execution, summary, createdAt: new Date().toISOString() };
