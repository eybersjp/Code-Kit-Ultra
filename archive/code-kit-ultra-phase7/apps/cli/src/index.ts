...
function writeMarkdownReport(dir: string, report: Awaited<ReturnType<typeof runVerticalSlice>>) {
  const md = [
    ...
    "## Routes", ...report.routes.map((r) => `- ${r.taskType}: ${r.adapterName} [${r.mode}]`), "",
    "## Execution", ...((report.execution || []).map((e: any) => `- ${e.taskType}: ${e.adapter} ok=${e.ok} dryRun=${e.dryRun} latency=${e.latencyMs ?? "n/a"}ms`))
  ].join("\n");
...
program.name("code-kit").description("Code Kit Ultra CLI").version("0.7.0");
...
program.command("init")
  ...
  .action(async (idea, options) => {
    ...
    (report.execution || []).forEach((e: any) => console.log(`- exec ${e.taskType}: ${e.adapter} ok=${e.ok} dryRun=${e.dryRun} latency=${e.latencyMs ?? "n/a"}ms`));
