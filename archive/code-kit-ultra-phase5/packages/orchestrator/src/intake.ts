...
export function deriveAssumptions(input: UserInput): Assumption[] {
  ...
  if (!/mobile|ios|android/i.test(input.idea)) assumptions.push({ id: "a3", text: "Assuming web-first responsive experience unless mobile app is explicitly required.", confidence: "medium" });
  return assumptions;
}
...
export function generateClarifyingQuestions(input: UserInput): ClarifyingQuestion[] {
  ...
  if (!/auth|login|users|roles/i.test(input.idea)) questions.push({ id: "q3", text: "Will the system need authentication and multiple user roles?", blocking: false });
  return questions;
}