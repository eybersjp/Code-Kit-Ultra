...
function getEffectiveWeight(
  ...
  const reliabilityFactor = 1 + ((getReliability(profile) - 0.5) * 2) * (policy.reliabilityInfluence ?? 0.35);
  const confidenceFactor = 1 + ((clamp01(vote.confidence) - 0.5) * 2) * (policy.confidenceInfluence ?? 0.65);
