...
import type { PlatformAdapter, RoutingPolicyConfig } from "../../shared/src/contracts";
import type { PersistentMemory, TaskType } from "../../shared/src/types";
import { getAdapterByName, mockAdapters } from "./mock-adapters";
...
export function selectAdapterForTask(
...
): { adapter: PlatformAdapter; reason: string } {
  ...
  const fallback = mockAdapters.find((adapter) => adapter.canHandle(taskType));
  if (!fallback) {
