...
interface ProjectMemory {
  ...
  executionHistory: Array<{ adapter: string; taskType: string; mode: string; ok: boolean; at: string }>;
}
...
export function loadProjectMemory(): ProjectMemory {
  ...
  if (!fs.existsSync(file)) {
    const seed: ProjectMemory = { acceptedAssumptions: [], successfulSkillMappings: {}, successfulAdapterSelections: {}, adapterSuccessCounts: {}, promotionHistory: [], executionHistory: [] };
...
export function recordExecution(adapter: string, taskType: string, mode: string, ok: boolean) {
  const mem = loadProjectMemory();
  mem.executionHistory.push({ adapter, taskType, mode, ok, at: new Date().toISOString() });
