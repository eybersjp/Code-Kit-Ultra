...
export function getModePolicy(mode: Mode): ModePolicy {
  switch (mode) {
    case "safe":
      return { mode, maxQuestions: 10, pauseGates: ["clarity", "scope", "architecture", "qa", "security", "cost", "deployment"], autoProceedNonCritical: false };
    case "god":
      return { mode, maxQuestions: 2, pauseGates: ["deployment"], autoProceedNonCritical: true };
    default:
      return { mode, maxQuestions: 4, pauseGates: ["architecture", "deployment"], autoProceedNonCritical: true };
  }
}
